...
function startGame() {
    ...
    if (game.status !== Game.STATUS.playing) document.removeEventListener("keydown", handleKey);
}
function handleKey(e) {
    ...
    score.textContent = game.getScore();
    if (game.getStatus() === Game.STATUS.lose) loseMessage.classList.remove("hidden");
    else if (game.getStatus() === Game.STATUS.win) winMessage.classList.remove("hidden");
...
class Game {
    ...
    setField() {
        ...
        for (const cell of flatedState)if (cell === 2048) this.status = Game.STATUS.win;
